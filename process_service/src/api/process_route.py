...
async def process_bin_files(
    ...
    try:
        ...
        parse_gps_result : ParseResult = await manager.parser_gps_messages()
...
async def process_bin_files(
    ...
    try:
        ...
        # await manager.parser_file_multiprocess()
        # parse_file_result = await manager.parser_all_file_mavlink()
        parse_file_result : ParseResult = await manager.parser_pure_python()
        if parse_file_result is None or parse_file_result.status == ParseStatus.FAILED: 
            raise HTTPException(status_code=500, detail='failed process this file')
        
        logger.info(f"File processed successfully | Total messages: {parse_file_result.count} | Time: {parse_file_result.duration}")
        
        response = parse_file_result
        background_tasks.add_task(remove_temp_file, temp_file_path)
