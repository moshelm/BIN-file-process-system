            ...
            for i, line in enumerate(jsonl_data.splitlines()):
                if line.strip():
                    try:
                        ...
                        logger.warning(
                            "Line %s: Invalid JSON format. Skipping.", i, extra={"line_error": line, "line_number": i}
                        )
                    except ValidationError as e:
                        logger.warning(
                            "Line %s: Data validation failed. Error: %s. Skipping.",
                            i,
                            e.json(),
                            extra={"line_error": line, "line_number": i},
                        )
                    except Exception as e:
                        logger.warning(
                            "Line %s: Unexpected error: %s. Skipping.",
                            i,
                            e,
                            extra={"line_error": line, "line_number": i},
                        )
