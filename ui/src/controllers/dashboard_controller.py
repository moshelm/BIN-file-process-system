...
class DashboardController:
    ...
    async def handle_file_upload(self, local_file_path: str):
        try:
            ...
            async for parse_result in self.api.get_information_about_parsers(self.config.bin_parsers_review, local_file_path):
...
def create_parser_result_card(json_string: str) -> ft.Card:
    try:
        ...
        status_color = ft.Colors.GREEN if status == "success" else ft.Colors.RED if status == "failed" else ft.Colors.AMBER
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ...
                        ft.Row([
                            ...
                            ft.Text(f"{duration:.2f} sec", weight=ft.FontWeight.BOLD, color=ft.Colors.ORANGE_ACCENT),
                        ]),
